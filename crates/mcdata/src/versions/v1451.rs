use crate::helpers::block_name_to_id_v1451::block_name_to_id;
...
pub(crate) fn register() {
    ...
    types::tile_entity_mut().add_converter_for_id(
        ...
        map_data_converter_func(|data, _from_version, _to_version| {
            ...
            data.insert(
                "blockState",
                block_flattening_v1450::get_new_nbt_for_id((block_id << 4) | block_data as u16),
            );
        }),
    );
            ...
            fn replace_ids(tag: &mut JCompound, key: &str) {
                ...
                if let JList::String(strings) = value {
                    for string in strings {
                        ...
                        // namespaced and non-namespaced blocks are both converted to namespaced in vanilla,
                        // but keep arbitrary NBT
                        if let Some(block) = block_name_to_id().get_mc(string) {
                            *string = block_flattening_v1450::get_new_name_for_id((*block as u16) << 4).to_owned();
                        } else {
                    ...
                    let new_list: Vec<_> = value
                        ...
                        .filter_map(|value| {
                            value.as_i16().map(|id| {
                                block_flattening_v1450::get_new_name_for_id(id as u16).to_owned()
                            })
                        })
                        .collect();
                ...
                for tile_tick in tile_ticks {
                    ...
                    if let Some(id) = id_val.as_i16() {
                        *id_val = JValue::String(
                        ...
                        // TODO: check that block names are always namespaced in vanilla
                ...
                if (0..=3).contains(&version) {
                    let result = layers
                        ...
                        .map(|layer| {
                            ...
                            let new_block_name = block_flattening_v1450::get_new_state_for_id_raw(
        ...
        map_data_converter_func(|data, _from_version, _to_version| {
            fn convert_to_block_state(data: &mut JCompound, path: &str) {
                if let Some(id) = data.get(path).and_then(|v| v.as_i16()) {
                    data.insert(
                        path,
                        block_flattening_v1450::get_new_nbt_for_id((id as u16) << 4),
                    );
                }
            }
